...
class BACnetClient:
    ...
    def get_object_list(self, device_params: dict) -> dict or False:
        if not isinstance(device_params, dict):
            return False
        object_dict = {'type': [], 'id': [], 'name': [], 'description': []}
        if not validate_ip(device_params['device_ip']):
            self.bc_logger.error('Device-IP incorrect!')
            return False
        if not validate_digit(device_params['obj_id'], 1, 4194303):
            self.bc_logger.error('Device-ID incorrect!')
            return False
        try:
            object_list = self.client.read(
                f"{device_params['device_ip']}/24 device {device_params['obj_id']} objectList")
            if len(object_list) > 0:
                for obj in object_list:
                    object_dict['type'].append(obj[0])
                    object_dict['id'].append(obj[1])
                    name = self.client.read(f"{device_params['device_ip']}/24 {obj[0]} {obj[1]} objectName")
                    if isinstance(name, (str, list)) and len(name) > 0:
                        object_dict['name'].append(name)
                    else:
                        object_dict['name'].append(None)
            return object_dict
        except Exception as e:
